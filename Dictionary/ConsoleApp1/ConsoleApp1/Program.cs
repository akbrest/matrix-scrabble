    ...
    while ((s = sr.ReadLine()) != null)
    {
        ...
        if (index > 1) { 
            ...
            if (secondWord.Contains(toFind[0]) || secondWord.Contains(toFind[1]) || secondWord.Contains(toFind[2]))
            {
                ...
                if (secondWord.Contains("ii сущ"))
                {
                    var containing = secondWord.IndexOf("ii сущ");
                    var element = secondWord.Substring(containing);
                    var elem = element.Substring(8);

                    var comma_end = elem.IndexOf(",") ;
                    if(comma_end <0) {
                        comma_end = elem.IndexOf(".");
                            }
                    var word = elem.Substring(0, comma_end).ToLower();
