...
const GameForm = () => {
  ...
  const handleChangeWord = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (e.target.value !== "") {
      status = (
        await dispatch(fetchWord({ language: language, word: e.target.value }))
      ).payload as boolean;
    } else {
      status = false;
    }
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (word || true) {
      ...
      const createdGame = (await dispatch(createGame(createGameRequest)))
        .payload as Game;
              ...
              <div className="create-form-block-length">
                ...
                <div className="form-label">
                  <Pagination>
                    ...
                    <Pagination.Item
                      active={6 === active}
                      onClick={() => changeAmount(6)}
                    >
                      {6}
                    </Pagination.Item>
                    <Pagination.Item
                      active={7 === active}
                      onClick={() => changeAmount(7)}
                    >
                      {7}
                    </Pagination.Item>
                  </Pagination>
