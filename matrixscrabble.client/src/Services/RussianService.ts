...
class RussianService implements ILanguageService {
  apiKey =
    'dict.1.1.20240111T152922Z.a4f3a2df2c67d15e.ac1af2305676c3ef964f0907440ee866d27f5494';
    ...
    public async GetWordStatus(word: string) {
        try {
          const value = await this.getDefinition(word);
          // Assuming the API returns an array of definitions
    ...
    public async CheckWords(data: [{ word: string, length: number, status: string, }]) {
        await data.forEach(async (element) => {
          element.status = await this.GetWordStatus(element.word);
